...
#    vout = cv2.VideoWriter(dstname, fourcc, 30.0, (640,480))
    length = int(vin.get(cv2.CAP_PROP_FRAME_COUNT))
    print('{}: {}'.format(filename, length))
    csvf = open(data['Name'][i]+'.csv', 'w')
    csvout='frame,bbx,bby,bbw,bbh,'
    for j in range(68):
        csvout += 'x_%d,y_%d,'%(j,j)
    csvout += 'pitch,yaw,roll'
    csvf.write(csvout+'\n');
        ...
        for k, d in enumerate(face_rects):
            ...
            if len(shape) == 68:
                ...
                for i in range(len(shape)):
                    csvout+=',%d,%d'%(shape[i][0], shape[i][1])
